...
static void f3_alpha_blend16_s( const UINT8 *alphas, UINT32 s )
{
	dval = alphas[s & 0x1f] | (alphas[(s>>5) & 0x3f]<<5) | (alphas[(s>>11) & 0x1f]<<11);
}

static void f3_alpha_blend16_d_lmt( const UINT8 *alphas, UINT32 s )
{
	int d=dval;
	if(d)
	{
		int c1,c2,c3;
		c1=alphas[s & 0x1f]+(d & 0x1f);
		if(c1&~0x1f) c1=0x1f;

		c2=alphas[(s>>5) & 0x3f]+((d>>5) & 0x3f);
		if(c2&~0x3f) c2=0x3f;

		c3=alphas[(s>>11) & 0x1f]+((d>>11) & 0x1f);
		if(c3&~0x1f) c3=0x1f;

		dval = c1 | (c2<<5) | (c3<<11);
	}
	else dval = alphas[s & 0x1f] | (alphas[(s>>5) & 0x3f]<<5) | (alphas[(s>>11) & 0x1f]<<11);
}

static void f3_alpha_blend16_d_nml( const UINT8 *alphas, UINT32 s )
{
	dval += alphas[s & 0x1f] | (alphas[(s>>5) & 0x3f]<<5) | (alphas[(s>>11) & 0x1f]<<11);
}
...
static void init_alpha_blend_func(void)
{
	...
	f3_alpha_blend_s = f3_alpha_blend16_s;
	f3_alpha_blend_d_lmt = f3_alpha_blend16_d_lmt;
	f3_alpha_blend_d_nml = f3_alpha_blend16_d_nml;
}
...
/*				if (dest_bmp->depth == 32) */
				{
					...
					UINT16 *dest0 = (UINT16 *)dest_bmp->line[sy]+sx;
