...
#if GCW0 /* DEBUG */
#define DEBUG_VISUAL	fprintf(stderr_file, "blit_swapxy:%d blit_flipx:%d blit_flipy:%d " \
			"visual_width:%d visual_height:%d " \
			"visual.min_x:%d visual.max_x: %d visual.min_y: %d visual.max_y: %d\n", \
			blit_swapxy, blit_flipx, blit_flipy, \
			visual_width, visual_height, \
			visual.min_x, visual.max_x, visual.min_y, visual.max_y); 
#endif
...
int osd_create_display(const struct osd_create_params *params, UINT32 *rgb_components)
{
	...
	/* Generally only useful for hardware which supports 8-bit palette modes ie. Not GCW-Zero */
...
static void update_visible_area(struct mame_display *display)
{
	...
	/* Handled by the GCW0 blitting code */
