...
void populate_table(struct memport_data *memport, int iswrite, offs_t start, offs_t stop, UINT8 handler)
{
	...
	if (l2start != 0)
	{
		...
		{
#ifdef GCW0
			// This was incredibly subtle bug to find
			// GCC 4.8.2 gets this wrong somehow
			UINT8 newsubtable = alloc_new_subtable(memport, tabledata, subindex);
			tabledata->table[l1start] = newsubtable;
			subindex = newsubtable;
#else
	...
	if (l2stop != l2mask)
	{
		...
		if (subindex < SUBTABLE_BASE)
		{
			...
			// GCC 4.8.2 gets this wrong somehow
			UINT8 newsubtable = alloc_new_subtable(memport, tabledata, subindex);
			tabledata->table[l1stop] = newsubtable;
			subindex = newsubtable;
#else
