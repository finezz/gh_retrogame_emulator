...
#ifdef GCW0
{
		...
		bounds.min_x = Machine->visible_area.min_x;
		bounds.max_x = Machine->visible_area.max_x;
		bounds.min_y = Machine->visible_area.min_y;
		bounds.max_y = Machine->visible_area.max_y;

		int orientation = drivers[game_index]->flags & ORIENTATION_MASK;

		if (orientation == ROT270 && (Machine->gamedrv->flags & ORIENTATION_SWAP_XY && ((Machine->orientation & ORIENTATION_SWAP_XY) == 0)))
		{
			if (Machine->drv->video_attributes & VIDEO_TYPE_VECTOR)
			{
		...
		else if (orientation == ROT90 && (Machine->gamedrv->flags & ORIENTATION_SWAP_XY && ((Machine->orientation & ORIENTATION_SWAP_XY) == 0)))
		{
			if (Machine->drv->video_attributes & VIDEO_TYPE_VECTOR)
			{
			    sizey = options.vector_width;
