void MY_FUNCTION (struct osd_bitmap *bitmap)
{
    DEST_PIXEL *dest = DEST;
    SRC_PIXEL *src = (SRC_PIXEL *)bitmap->line[visual.min_y] + visual.min_x;
    int src_offset = ((bitmap->line[1] - bitmap->line[0])>>SRC_SHIFT)-SRC_WIDTH;
    int src_width = (bitmap->line[1] - bitmap->line[0])>>SRC_SHIFT;
    int src_inc = SRC_INC;
    int xstep,ystep,i,j;
    int x,y=SRC_HEIGHT;
