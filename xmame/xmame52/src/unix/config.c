...
#endif
#ifdef SAVESTATE_LOAD
static char savegame[16];
#endif
#ifdef SPEEDHACK
...
#endif
#ifdef SAVESTATE_LOAD
static int check_savegame(struct rc_option *option, const char *arg, int priority)
{
	options.savegame=(char)arg[0];
	if ((options.savegame >= 'a' && options.savegame <= 'z') || (options.savegame >= '0' && options.savegame <= '9'))
		return 0;
	fprintf(stderr, "Bad game state '%c'; aborting\n", arg[0]);
	options.savegame = 0;
	return -1;
}
#endif
...
static struct rc_option opts[] = {
   ...
   { "skip_warnings",	NULL,			rc_bool,	&options.skip_warnings,
     "0",		0,			0,		NULL,
...
#if FRONTEND
   { "frontend",	"fe",			rc_string,	&frontend,
     NULL,		0,			0,		NULL,
     "Frontend executable to run on exit" },
#endif
#ifdef SAVESTATE_LOAD
   { "savestate", "ss", rc_string, &savegame, NULL, 0, 0, check_savegame, "Save state to load [A-Z0-9]" },
#endif
