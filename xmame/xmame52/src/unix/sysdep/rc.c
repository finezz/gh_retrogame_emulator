...
#ifdef OPEN_DINGUX
	    /*
	     * Something screwy with the strtof/strtod implementation on Open Dingux
	     * The GCW-Zero version seems to work, the mips32 OD version does not
	     */
	    sscanf(arg, "%f", &x);
            int res = rc_verify(option, x);
