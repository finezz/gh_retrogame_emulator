...
void osd_customize_inputport_rotated_defaults(unsigned int rotation, struct ipd *defaults)
{
	if (rotation == ROT90)
	{
	    ...
	    {
		    if (defaults->type == IPT_JOYSTICK_UP) seq_set_1(&defaults->seq, KEYCODE_RIGHT); 
		    if (defaults->type == IPT_JOYSTICK_DOWN) seq_set_1(&defaults->seq, KEYCODE_LEFT); 
		    if (defaults->type == IPT_JOYSTICK_LEFT) seq_set_1(&defaults->seq, KEYCODE_UP); 
		    if (defaults->type == IPT_JOYSTICK_RIGHT) seq_set_1(&defaults->seq, KEYCODE_DOWN); 

		    if (defaults->type == IPT_UI_UP) seq_set_1(&defaults->seq, KEYCODE_RIGHT); 
		    if (defaults->type == IPT_UI_DOWN) seq_set_1(&defaults->seq, KEYCODE_LEFT); 
		    if (defaults->type == IPT_UI_LEFT) seq_set_1(&defaults->seq, KEYCODE_UP); 
		    if (defaults->type == IPT_UI_RIGHT) seq_set_1(&defaults->seq, KEYCODE_DOWN); 

		    // How to set defaults for analog controls?
		    //if (defaults->type == IPT_AD_STICK_X) seq_set_2(&defaults->seq, KEYCODE_UP, KEYCODE_DOWN); 
		    //if (defaults->type == IPT_AD_STICK_Y) seq_set_2(&defaults->seq, KEYCODE_LEFT, KEYCODE_RIGHT); 
		    defaults++;
	    }
	}
	else
	{
	    while (defaults->type != IPT_END)
	    {
		    if (defaults->type == IPT_JOYSTICK_UP) seq_set_1(&defaults->seq, KEYCODE_LEFT); 
		    if (defaults->type == IPT_JOYSTICK_DOWN) seq_set_1(&defaults->seq, KEYCODE_RIGHT); 
		    if (defaults->type == IPT_JOYSTICK_LEFT) seq_set_1(&defaults->seq, KEYCODE_DOWN); 
		    if (defaults->type == IPT_JOYSTICK_RIGHT) seq_set_1(&defaults->seq, KEYCODE_UP); 

		    if (defaults->type == IPT_UI_UP) seq_set_1(&defaults->seq, KEYCODE_LEFT); 
		    if (defaults->type == IPT_UI_DOWN) seq_set_1(&defaults->seq, KEYCODE_RIGHT); 
		    if (defaults->type == IPT_UI_LEFT) seq_set_1(&defaults->seq, KEYCODE_DOWN); 
		    if (defaults->type == IPT_UI_RIGHT) seq_set_1(&defaults->seq, KEYCODE_UP); 
		    defaults++;
	    }
	}
}
...
void osd_customize_inputport_defaults(struct ipd *defaults)
{
	...
	while (defaults->type != IPT_END)
	{
		...
		if (defaults->type == IPT_UI_CANCEL) seq_set_3(&defaults->seq, KEYCODE_ESC, KEYCODE_TAB, KEYCODE_BACKSPACE); 
		if (defaults->type == IPT_UI_PAUSE) seq_set_2(&defaults->seq, KEYCODE_TAB, KEYCODE_BACKSPACE); 
		if (defaults->type == IPT_UI_PAUSE) seq_set_2(&defaults->seq, KEYCODE_TAB, KEYCODE_BACKSPACE);
		if (defaults->type == IPT_UI_CONFIGURE) seq_set_2(&defaults->seq, KEYCODE_ENTER, KEYCODE_TAB); 
		if (defaults->type == IPT_UI_SHOW_FPS) seq_set_2(&defaults->seq, KEYCODE_ENTER, KEYCODE_BACKSPACE); 
		if (defaults->type == IPT_START1) seq_set_2(&defaults->seq, KEYCODE_ESC, KEYCODE_TAB);
		if (defaults->type == IPT_COIN1)  seq_set_2(&defaults->seq, KEYCODE_ESC, KEYCODE_BACKSPACE);
		if (defaults->type == IPT_UI_SAVE_STATE) seq_set_2(&defaults->seq,KEYCODE_ENTER,KEYCODE_LEFT);
		if (defaults->type == IPT_UI_LOAD_STATE) seq_set_2(&defaults->seq,KEYCODE_ENTER,KEYCODE_RIGHT);
		if (defaults->type == IPT_UI_SNAPSHOT) seq_set_2(&defaults->seq,KEYCODE_ENTER,KEYCODE_DOWN);
		...
		if (defaults->type == (IPT_BUTTON3 | IPF_PLAYER1)) seq_set_1(&defaults->seq,KEYCODE_LSHIFT);
		if (defaults->type == (IPT_BUTTON4 | IPF_PLAYER1)) seq_set_1(&defaults->seq,KEYCODE_SPACE);
		defaults++;
	}
#else
	if (use_hotrod)
