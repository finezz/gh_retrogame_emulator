     ...
     int write)
{
	...
	switch (filetype)
	{
		...
		case OSD_FILETYPE_SAMPLE:
		    ...
		    for(i=0; i < rompathc; i++)
		    {
		    /* try <$XMAMEROOT>/roms/gamename.zip */
			snprintf(name, MAXPATHL, "%s/roms/%s.zip", home_dir, gamename);
			if (open_zip_file(f, name, filename))
			    break;
			...
			/* try <$XMAMEROOT>/samples/gamename.zip */
			snprintf(name, MAXPATHL, "%s/samples/%s.zip", home_dir, gamename);
			if (open_zip_file(f, name, filename))
			    break;
		...
		case OSD_FILETYPE_CONFIG:
		    snprintf(name, MAXPATHL, "%s/cfg/%s.cfg", home_dir, gamename);
		    f->file = fopen(name,write ? "w" : "r");
		    break;
		case OSD_FILETYPE_STATE:
		    snprintf(name, MAXPATHL, "%s/sta/%s-%s.sta", home_dir, gamename, filename);
		    f->file = fopen(name,write ? "w" : "r");
		    break;
		case OSD_FILETYPE_NVRAM:
		    snprintf(name, MAXPATHL, "%s/nvram/%s.nv", home_dir, gamename);
		    f->file = fopen(name,write ? "w" : "r");
		    break;
		case OSD_FILETYPE_MEMCARD:
		    snprintf(name, MAXPATHL, "%s/memcard/%s.mem", home_dir, filename);
		    f->file = fopen(name,write ? "w" : "r");
		    break;
