...
static int direct_mapped_15bpp = 0;
...
/* Disabled artwork, backdrop, overlay and bezels for GCW0 */
   ...
   { "flipy",		"fy",			rc_set_int,	&options.flipy,
     ...
     "Flip Y axis" },
#ifdef GCW0
   // Added to this version of MAME
   { "autoror",		NULL,			rc_bool,	&options.autoror,
     "0",		0,			0,		NULL,
   ...
   { "packed_gfx",      "pgfx",                 rc_bool,        &options.use_packed_gfx,
     "0",               0,                      0,              NULL,
   ...
   int fps, int attributes, int orientation)
{
...
#ifdef GCW0
   if (sysdep_create_display(visual_width, visual_height, bitmap_depth, direct_mapped_15bpp) != OSD_OK)
#else
   ...
   if (bitmap_depth>=15) fprintf(stderr_file,"Using %ubpp video mode\n",bitmap_depth);
...
#else
      if ((Machine->drv->video_attributes & VIDEO_RGB_DIRECT) && bitmap_depth == 16)
      {
			i = 0;
