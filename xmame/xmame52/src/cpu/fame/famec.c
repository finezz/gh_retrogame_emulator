...
static EXTRA_INLINE int32_t interrupt_chk(void)
{
    ...
    if (FAME_CONTEXT.interrupts[0])
    {
        ...
        if (irql == 7)		/* NMI */
            return irql;
        else if (irql > flag_I /* this perhaps just should be M68K_PPL not flag_I - Slaanesh */)
	{
#ifdef FAME_PREVIOUSPC
	    /*
...
/***************************************************************************/
void FAME_API(get_context)(FAME_CONTEXT *context)
{
    memcpy(context,&FAME_CONTEXT,sizeof(FAME_CONTEXT));
}
...
/* No retorna ningun valor                                                 */
/***************************************************************************/
void FAME_API(set_context)(const FAME_CONTEXT *context)
{
    memcpy(&FAME_CONTEXT,context,sizeof(FAME_CONTEXT));
    famec_SetBanks();
}
