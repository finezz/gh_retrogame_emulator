...
unsigned short sjis_to_utf16(unsigned short in)
{
    return sjis_2_utf16[in - 0x8140];
}

int utf16_2_utf8( unsigned char dst[4], unsigned short src )
{
    if (src & 0xff80){
        if (src & 0xf800){
      ...
      if(ch < 128){
        ...
        if(chFont){
          uint8_t buf[4];
			    ...
			    dstrect.y = y;
          shift = spacing * 2;
          utf16_2_utf8(buf, sjis_to_utf16(ch));
          SDL_Surface *p = TTF_RenderUTF8_Blended(chFont, (const char*)buf, fcol);
			    SDL_BlitSurface(p, NULL, sdl_screen, &dstrect);
          SDL_FreeSurface(p);
        }
      }
		}
		...
		{	// fixed spacing
			...
			if(ch == ' ' && shrink_spaces){	// 10.5 px for spaces - make smaller than they really are - the default
				x+= (SCALE == 1) ? 5 : 10;
				if(i & 1){
          x++;
        }
