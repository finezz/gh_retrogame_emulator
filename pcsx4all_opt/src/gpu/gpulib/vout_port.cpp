...
// GPU_BlitWW
void GPU_Blit320(const void* src, u16* dst16, bool isRGB24)
{
	u32 uCount;
	if (!isRGB24)
	{
...
void vout_update_line(int x, int y, uint16_t *mem, int len)
{
	int x0 = x;
	int y0 = y;
	...
	u16* src16 = mem;
	u16* dst16 = &dma_ptr[(y0 * 1024) + x0];

	switch(w0){
	case 256:
	  GPU_Blit256(src16, dst16, isRGB24);
		break;
	case 368:
		if(use_clip_368 == false || isRGB24){
		  GPU_Blit368(src16, dst16, isRGB24);
		}
		else{
			GPU_Blit368_clip(src16, dst16);
		}
		break;
	case 320:
		GPU_Blit320(src16, dst16, isRGB24);
		break;
	case 384:
		GPU_Blit384(src16, dst16, isRGB24);
		break;
	case 512:
		GPU_Blit512(src16, dst16, isRGB24);
		break;
	case 640:
		GPU_Blit640(src16, dst16, isRGB24);
		break;
	}
	dma_flip();
}
